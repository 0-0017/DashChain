...
def load_data(env_data_dict):
    ...
    # save data to cache
    data = env_data_dict
    with open('data_cache.csv', 'w', newline='') as f:
        writer = csv.writer(f)
...
def train_model(env_data_dict):
    ...
    for row in np_data:
        ...
        model.learn(total_timesteps=120_000)  # Adjust as needed
    ...
    mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=300, deterministic=True)
...
def hyper_tuning():
    ...
    def objective(trial):
        ...
        # Evaluate
        mean_reward, _ = evaluate_policy(model, env, n_eval_episodes=30, deterministic=True)
        return mean_reward
