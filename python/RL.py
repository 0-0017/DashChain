...
class DashEnv(gym.Env):
    ...
    def step(self, action):
        ...
        terminated = self.current_step >= 300  # 'False' True if governance cycle ends
        truncated = False
        info = {}
    ...
    def reset(self, seed=None, options=None):
        ...
        self.voting_period = np.random.randint(5, 30)
        self.num_delegates = np.random.randint(5, 100)
        self.stake_variance = np.random.uniform(0, 1)
        self.tx_volume = np.random.randint(100, 1000)

        return np.array([self.voting_period, self.num_delegates, self.stake_variance, self.tx_volume], dtype=np.float32), {}
    ...
    def comp_reward(self):
        ...
        optimal_period = 7 * 86400 / (1 + self.votes_per_period / 100 + self.blocks_created / 1e6)
